// // navbar toggling
// const navbarShowBtn = document.querySelector('.navbar-show-btn');
// const navbarCollapseDiv = document.querySelector('.navbar-collapse');
// const navbarHideBtn = document.querySelector('.navbar-hide-btn');

// navbarShowBtn.addEventListener('click', function(){
//     navbarCollapseDiv.classList.add('navbar-show');
// });
// navbarHideBtn.addEventListener('click', function(){
//     navbarCollapseDiv.classList.remove('navbar-show');
// });

// // changing search icon image on window resize
// window.addEventListener('resize', changeSearchIcon);
// function changeSearchIcon(){
//     let winSize = window.matchMedia("(min-width: 1200px)");
//     if(winSize.matches){
//         document.querySelector('.search-icon img').src = "images/search-icon.png";
//     } else {
//         document.querySelector('.search-icon img').src = "images/search-icon-dark.png";
//     }
// }
// changeSearchIcon();

// // stopping all animation and transition
// let resizeTimer;
// window.addEventListener('resize', () =>{
//     document.body.classList.add('resize-animation-stopper');
//     clearTimeout(resizeTimer);
//     resizeTimer = setTimeout(() => {
//         document.body.classList.remove('resize-animation-stopper');
//     }, 400);
// });

// document.getElementById('sortSelect').addEventListener('focus', function() {
//     this.options[0].hidden = true;
// });

// document.getElementById('genderSelect').addEventListener('focus', function() {
//     this.options[0].hidden = true;
// });






// // delete functionaltiy

var deleteBtn = document.getElementById("btn");
var elements = document.querySelectorAll("#delBtn");
var table = document.querySelectorAll(".table");

elements.forEach(function(elems) {
    if (elems.style.visibility === '') {
        elems.style.visibility = 'hidden';
    }

    elems.addEventListener('click',function(){
        alert("Deleted")
    })
});

deleteBtn.addEventListener("click", function(){
    elements.forEach(function(elems){
        if(getComputedStyle(elems).visibility === 'hidden'){
            elems.style.visibility = 'visible';
            deleteBtn.innerHTML = "Hide";
            deleteBtn.style.backgroundColor = "rgb(37, 59, 255)";
        } else {
            elems.style.visibility = 'hidden';
            deleteBtn.innerHTML = "Delete";
            deleteBtn.style.backgroundColor = "rgb(255, 41, 41)";
        }
    });    
});


